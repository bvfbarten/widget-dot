...
exports.Widget = function (args) {
	...
	var extraData = self.$el.dataset['data'];
	if (extraData) {
	...
	delete(self.$el.dataset['data']);
    }
    self.compileTemplate = function () {
	if (this.templateId) {
	    var node = document.querySelector(this.templateId).content.cloneNode(true);
	    this.template = "";
	    for (var i = 0; i < node.children.length; i += 1) {
		this.template += node.children[0].outerHTML;
	    }
	}
	...
	window.setTimeout(function() {
		...
		morphdom(self.$el, self.renderString(data), {childrenOnly: true});
	    }
	}, 0);
    }
    //--end functions
